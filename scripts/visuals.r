...
fit <- .lm.fit(x, imat)
rss <- colSums(fit$residuals)
mrss <- sqrt(rss / (n - 1)) # mean residual sum of squares
...
# create surface rendering of clusters
brain <- renderSurfaceFunction( list(mask), list(roi), alphasurf = .5, smoothsval = 1, smoothfval = 1)
